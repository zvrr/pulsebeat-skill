...
def build(args):
    ...
    manifest={'language':language,'template':'pulsebeat-editorial-v3','analysisSha256':hashlib.sha256(source.read_bytes()).hexdigest(),'interpretationSha256':hashlib.sha256(note.encode()).hexdigest(),'htmlSha256':hashlib.sha256(out.read_bytes()).hexdigest(),'containsAgentInterpretation':True,'shareFile':share.name}
