...
PROVIDERS={'kugou':'酷狗音乐','qqmusic':'QQ 音乐','netease':'网易云音乐'}
...
def overview(data,note):
    ...
    for section in re.split(r'\n## ',note)[1:]:
        ...
        if body:
            ...
            findings.append({'title':plain(title[1] if title else section.splitlines()[0]),'detail':plain(body[0])[:180],'evidence':'详见完整 AI 解读及统计底稿'})
    return {'headline':findings[0]['title'] if findings else '每一次记录，都有自己的节奏。','summary':findings[0]['detail'] if findings else (paragraphs[0][:180] if paragraphs else '等待个人数据解读。'),'findings':findings[:3],'next_steps':[{'title':'持续记录，再观察变化','detail':'结合报告中提出的问题，继续积累同口径数据；只有来源齐全、日期可对齐的记录才参与比较。'},{'title':'带着场景理解音乐','detail':'区分收藏偏好、实际收听和发生时段；不要把同一天的共同变化当成因果。'}]}
def ring(value,mean):
    ...
    return '<svg viewBox="0 0 280 280" role="img" aria-label="恢复分数，刻度零至一百">'+content+'</svg>'
def chart(points,color,unit='',maximum=None,bar=False,mini=False):
    ...
    if not pairs:return '<p class="subtle">暂无可画图的观测</p>'
    ...
    flush()
    return f'<svg class="{"spark" if mini else "series"}" viewBox="0 0 440 {50 if mini else 160}" role="img" aria-label="{esc(unit)}，{start} 至 {end}，{len(pairs)} 次观测">'+''.join(parts)+'</svg>'
def series_panels(data):
    health=data.get('healthDaily',[]);music=data.get('musicDaily',{})
    series=[(LABELS[key],[(r['date'],r.get(key)) for r in health],COLORS[key],unit,cap,False) for key,unit,cap in [('recovery','分',100),('sleep_hours','小时',None),('hrv_ms','ms',None),('resting_hr','bpm',None)]]
    series += [(PROVIDERS.get(p,p)+' · 每日收听',[(r['date'],r['seconds']/60) for r in rows if numeric(r.get('seconds'))],COLORS['music'],'分钟',None,True) for p,rows in music.items()]
    ...
    for window in ['30','7','all']:
        ...
        for label,points,color,unit,cap,bar in series:
            ...
            tiles.append('<article class="chart"><div class="chart-label">'+esc(label)+'<span>'+str(sum(numeric(v) for _,v in points))+' 次观测 · '+unit+'</span></div>'+chart(points,color,unit,cap,bar)+'<details><summary>查看日期与数值</summary><table>'+table+'</table></details></article>')
        panels.append('<div class="trend-grid" data-window="'+window+'"'+(' hidden' if window!='30' else '')+'>'+''.join(tiles)+'</div>')
    return ''.join(panels)
def render(data,interpretation='',insights=None,share_file='share.svg'):
    info=insights or overview(data,interpretation)
    title=str(info.get('headline','读懂自己的节奏。'));summary=str(info.get('summary',''))
    ...
    metrics=[]
    for key,unit in [('recovery','分'),('sleep_hours','h'),('hrv_ms','ms'),('resting_hr','bpm')]:
        ...
        metrics.append('<article class="metric" style="--accent:'+COLORS[key]+'"><div class="metric-label"><i></i>'+LABELS[key]+'</div><div class="value">'+esc(fmt(v,2 if key in ['sleep_hours','hrv_ms'] else 0))+'<small>'+unit+'</small></div><div class="comparison">观测均值 '+esc(fmt(mean,2))+' '+unit+' · n='+esc(metric.get('n',0))+'<br><b>最近值差 '+esc(compare)+' '+unit+'</b></div>'+chart(points,COLORS[key],unit,100 if key=='recovery' else None,mini=True)+'</article>')
    providers=[]
    for p,cn in PROVIDERS.items():
        c=data.get('musicCoverage',{}).get(p,{});available=data.get('providerAvailability',{}).get(p)=='available'
        description=(' → '.join(c.get('range') or [])+' · '+str(c.get('days',0))+' 天') if c else ('红心偏好，不代表实际收听' if available else '未取得个人授权数据')
        total=('<strong>'+esc(fmt(c.get('mean_minutes')))+'<small> 分钟/日</small></strong>') if c else '<strong class="unavailable">'+('偏好快照' if available else '尚未连接')+'</strong>'
        providers.append('<div class="provider"><span class="provider-icon">'+{'kugou':'KG','qqmusic':'QQ','netease':'NC'}[p]+'</span><div><b>'+cn+'</b><p>'+esc(description)+'</p></div>'+total+'</div>')
    associations=[]
    for row in a:
        state={'insufficient_data':'样本不足，不推断方向','exploratory_only':'仅探索，不代表因果','no_variation':'数据缺少变化'}.get(row.get('status'),'暂无结论')
        associations.append('<tr><td>'+esc(PROVIDERS.get(row.get('provider'),row.get('provider')))+'</td><td>'+esc(LABELS.get(row.get('outcome'),row.get('outcome')))+'</td><td>'+esc(row.get('n'))+'</td><td>'+esc(fmt(row.get('spearman_r'),3))+'</td><td><span class="status">'+esc(state)+'</span></td></tr>')
...
def share_card(data,info):
    ...
    cards=[]
    for i,(key,label) in enumerate([('recovery','恢复分数'),('sleep_hours','主睡眠 · h'),('hrv_ms','HRV · ms')]):
    ...
    n=primary.get('n',0);provider=PROVIDERS.get(primary.get('provider'),'音乐')
    return f'<svg xmlns="http://www.w3.org/2000/svg" width="900" height="780" viewBox="0 0 900 780" role="img"><title>PulseBeat 个人分享摘要</title><rect width="900" height="780" rx="38" fill="#141b1b"/><circle cx="895" cy="70" r="180" fill="none" stroke="#34412d"/><g font-family="Avenir Next, PingFang SC, sans-serif"><text x="72" y="82" fill="#c3d1b7" font-size="25" font-weight="700">pulsebeat.</text><text x="72" y="120" fill="#879782" font-size="12">MY RHYTHM / {esc(str(data.get("generatedAt",""))[:10])}</text>{title_svg}<line x1="72" y1="455" x2="828" y2="455" stroke="#344237"/>{"".join(cards)}<text x="72" y="669" fill="#b0baad" font-size="17">{esc(provider)}与次日恢复 · {n} 对配对 · 个人观察不代表因果</text><text x="72" y="713" fill="#788675" font-size="13">精选摘要，不含身份资料或原始记录。不是医疗诊断。</text></g></svg>'
def build(args):
    ...
    share.write_text(share_card(data,info),encoding='utf-8');os.chmod(share,0o600)
    out.write_text(render(data,note,info,share.name),encoding='utf-8');os.chmod(out,0o600)
    manifest={'template':'pulsebeat-editorial-v2','analysisSha256':hashlib.sha256(source.read_bytes()).hexdigest(),'interpretationSha256':hashlib.sha256(note.encode()).hexdigest(),'htmlSha256':hashlib.sha256(out.read_bytes()).hexdigest(),'containsAgentInterpretation':True,'shareFile':share.name}
