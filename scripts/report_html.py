...
def series_panels(data):
    ...
    return ''.join(panels)
def render(data,interpretation='',insights=None,share_file='share.svg',lang='auto'):
...
def build(args):
    ...
    out.write_text(render(data,note,info,share.name,language),encoding='utf-8');os.chmod(out,0o600)
    manifest={'language':language,'template':'pulsebeat-editorial-v2','analysisSha256':hashlib.sha256(source.read_bytes()).hexdigest(),'interpretationSha256':hashlib.sha256(note.encode()).hexdigest(),'htmlSha256':hashlib.sha256(out.read_bytes()).hexdigest(),'containsAgentInterpretation':True,'shareFile':share.name}
