...
def build(args):
    ...
    from share_cards import generate as generate_series
    series=generate_series(data,info,json.loads(pathlib.Path(args.media).read_text()) if getattr(args,'media',None) else None,out.with_name(out.stem+'-share-series'),language)
    manifest={'shareSeries':str(series.relative_to(out.parent)),'language':language,'template':'pulsebeat-integrated-v4','analysisSha256':hashlib.sha256(source.read_bytes()).hexdigest(),'interpretationSha256':hashlib.sha256(note.encode()).hexdigest(),'htmlSha256':hashlib.sha256(out.read_bytes()).hexdigest(),'containsAgentInterpretation':True,'shareFile':share.name}
